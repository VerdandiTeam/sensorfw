...
HybrisBackendBinderAidl::HybrisBackendBinderAidl(HybrisManager *manager, QObject *parent)
    : HybrisBackendBinder(manager, parent)
{
}
    ...
    if (gbinder_servicemanager_is_present(sm)) {
        ...
        GBinderRemoteObject *remote =
            gbinder_servicemanager_get_service_sync(sm,
                SENSOR_BINDER_SERVICE_NAME_AIDL, NULL);
        if (remote) {
            ret = true;
        }
    }
...
void HybrisBackendBinderAidl::eventReaderThreadImpl()
{
...
void HybrisBackendBinderAidl::getSensorList()
{
    ...
    reply = gbinder_client_transact_sync_reply(m_client, GET_SENSORS_LIST_AIDL, NULL, &status);
...
void HybrisBackendBinderAidl::binderDied(GBinderRemoteObject *, void *user_data)
{
    HybrisBackendBinderAidl *conn =
                    static_cast<HybrisBackendBinderAidl *>(user_data);
